...
const createCartItemTemplate = (item, index) => {
  ...
  return `
        <div class="flex items-center bg-white p-4 rounded-xl shadow-lg border border-gray-200">
            <img src="/asset/images/item-placeholder-${
              item.productId // Menggunakan productId untuk konsistensi gambar
            }.jpg" alt="${
    item.name
            ...
            <div class="flex-grow">
                ...
                <p class="text-gray-600">${formatRupiah(item.price)} x ${
    item.quantity
  }</p>
...
window.checkoutToWhatsapp = () => {
  ...
  message += `------------------------\n`;
  message += `*Perkiraan Total: ${formatRupiah(
    total
