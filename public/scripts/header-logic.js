...
// ===============================================
// Helper Functions: Direct Overflow Control (SESUAI PERMINTAAN ANDA)
// ===============================================
...
window.openProductModal = (id, name, price, imagesJson) => {
  ...
  const productModal = document.getElementById("product-detail-modal");
  const modalProductNameHeader = document.getElementById(
    "modal-product-name-header"
  );
  const modalProductName = document.getElementById("modal-product-name");
  const modalProductPrice = document.getElementById("modal-product-price");
  const modalProductImageMain = document.getElementById(
    "modal-product-image-main"
  );
  const modalImageThumbnails = document.getElementById(
  ...
  if (modalProductNameHeader)
    modalProductNameHeader.textContent = `Opsi Produk: ${name}`;
  ...
  if (modalImageThumbnails) {
    ...
    const finalImages =
      images.length > 0
        ? images
        : [{ thumb: defaultImageUrl, full: defaultImageUrl }];
...
const handleAddToCart = () => {
  if (!currentProductId || !currentProductPrice) {
    alert("Error: Gagal menambahkan produk. Data produk tidak lengkap.");
    return;
  }
  ...
  const customNumber =
    document.getElementById("custom-number")?.value.trim() || "";
  const quantity =
...
const handleSearchSubmit = (inputElement) => {
  ...
  if (query.length > 2) {
    ...
    alert("Masukkan minimal 3 karakter untuk melakukan pencarian.");
    inputElement.focus();
  }
};
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const productModal = document.getElementById("product-detail-modal");
  const productCloseBtn = document.getElementById(
  ...
  const faqLinks = document.querySelectorAll(
    '.open-faq-modal, a[href="#faq-modal"]'
  );
  ...
  const categorySidebarOverlay = document.getElementById(
  ...
  if (categorySidebarOverlay) {
    categorySidebarOverlay.addEventListener(
      "click",
      window.closeCategorySidebar
    );
  }
  ...
  if (productCloseBtn)
    productCloseBtn.addEventListener("click", window.closeProductModal);
  if (productBackdrop)
    productBackdrop.addEventListener("click", window.closeProductModal);
  ...
  document.addEventListener("keydown", (e) => {
    if (e.key === "Escape") {
      ...
      if (
        categorySidebar &&
        !categorySidebar.classList.contains("-translate-x-full")
      ) {
        window.closeCategorySidebar();
        return;
      }
      ...
      if (
        searchOverlayCheck &&
        !searchOverlayCheck.classList.contains("hidden")
      ) {
        const closeSearchModalAnimated = () => {
